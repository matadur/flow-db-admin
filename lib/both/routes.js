...
const pubSettings = Meteor.settings.public
const def_title_suffix = `${pubSettings.company.abbrev} Admin`


var fadminRoutes = FlowRouter.group({
  name: "AdminController",
  prefix: Meteor.settings.public.adminPrefix,
 ...
 triggersEnter: [
  	function(context) {
      triggerRouteMethod('AdminController')
			document.title = `${def_title_suffix}`
...
fadminRoutes.route('/',{
	...
	triggersEnter: [
	 function(context){
    triggerRouteMethod('adminDashboard')
		document.title = `Dashboard | ${def_title_suffix}`
...
fadminRoutes.route('/view/:collectionName',{
	triggersEnter: [
		function(context){
    triggerRouteMethod('admin-view')
		document.title = `${context.params.collectionName} - View | ${def_title_suffix}`
...
fadminRoutes.route('/new/:collectionName',{
    ...
    triggerRouteMethod('admin-new')
		document.title = `${context.params.collectionName} - New | ${def_title_suffix}`
		Session.set('admin_title', context.params.collectionName);
	...
	{	if(params.collectionName == 'Users')
			BlazeLayout.render('fAdminLayout',{main: AdminConfig.users.newTemplate});
		else
...
fadminRoutes.route('/edit/:collectionName/:_id',{
	triggersEnter: [function(context){
    triggerRouteMethod('admin-edit')
		document.title = `${context.params.collectionName} - Edit | ${def_title_suffix}`
		Session.set('admin_title', context.params.collectionName);
	...
	{
		if(params.collectionName == 'Users')
			BlazeLayout.render('fAdminLayout',{main: AdminConfig.users.editTemplate});
		else
...
function triggerRouteMethod (name) {
  if (AdminConfig && AdminConfig.onTriggerMethod) {
    AdminConfig.onTriggerMethod()
  }
}
